  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    s3Bucket.addToResourcePolicy(
      new iam.PolicyStatement({
        ...
        // ADICIONE ISSO: Garante que apenas a SUA distribuição acesse o bucket
        conditions: {
          StringEquals: {
            'AWS:SourceArn': `arn:aws:cloudfront::${this.account}:distribution/${cloudFrontWebDistribution.distributionId}`
          }
        }
      })
    )
